...
export const appointmentConfirmation = async (req, res, next) => {
    ...
    .select('email')
    .eq('id', client_id)
    console.log("client email obtained ......")
    ...
    const mailOptions = {
        ...
        "h:X-Mailgun-Variables": JSON.stringify({
            ...
            link : "www.test.com" // ! we need to get the link from the frontend
        })
    };
...
export const clientBookedAppointment = async (req, res) => {
    ...
    const { client_id, schedule_day, schedule_start, schedule_end, schedule_link } = req.body;
    ...
    const mailOptions = {
        from: 'festus from appointly <festus@email.appointly.co>',
        to: 'covodo9489@laluxy.com', // ! get details from the backend
